...
VULNS_SCANNER () {
	...
	cd /root/OK-VPS/tools && wget https://github.com/chaitin/xray/releases/download/1.9.11/xray_linux_amd64.zip && unzip xray_linux_amd64.zip && cp -r xray_linux_amd64 /usr/local/bin && cd /usr/local/bin && cp -r xray_linux_amd64 /usr/local/bin;
