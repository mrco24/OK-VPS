...
DNS_RESOLVER () {
	...
	cd /root/OK-VPS/tools && git clone https://github.com/blechschmidt/massdns.git > /dev/null 2>&1 && cd massdns && apt install make && make && make install > /dev/null 2>&1 && ln -s /root/OK-VPS/tools/massdns/bin/massdns /usr/local/bin/;
...
WEB_CRAWLING () {
	...
	cd /root/OK-VPS/tools && git clone https://github.com/xnl-h4ck3r/waymore.git /opt/waymore || git -C /opt/waymore pull && pip3 install -r /opt/waymore/requirements.txt && ln -s /opt/waymore/waymore.py /usr/local/bin/waymore && chmod +x /usr/local/bin/waymore > /dev/null 2>&1;
...
HTTP_PARAMETER () {
	...
	echo -e ${BLUE}"[HTTP PARAMETER DISCOVERY]" ${RED}"Arjun installation in progress ...";
        pip3 install arjun;
        cd /root/OK-VPS/tools && git colne https://github.com/s0md3v/Arjun.git && cd Arjun && pip3 install arjun && python3 setup.py install;
...
JS_HUNTING () {
	...
	go install github.com/003random/getJS/v2@latest > /dev/null 2>&1 && ln -s ~/go/bin/getJS /usr/local/bin/;
...
USEFUL_TOOLS () {
	...
	go install -v github.com/PentestPad/subzy@latest > /dev/null 2>&1 && ln -s ~/go/bin/subzy /usr/local/bin/;
